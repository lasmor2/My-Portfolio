      ...
      <div className="flex md:hidden flex-col w-full pt-28 pb-16 gap-6">
        ...
        <div className="flex justify-center">
          <div className="relative w-60">
            <div className="absolute -inset-3 rounded-3xl bg-foreground/5 blur-xl pointer-events-none" />
            <div className="relative w-full aspect-3/4 rounded-3xl overflow-hidden border border-border/50 shadow-2xl shadow-foreground/10">
              <Image
            ...
            {heroStats.map((stat, i) => (
              <div
                key={stat.value}
                className={`absolute flex items-center gap-1.5 px-3 py-2 rounded-xl bg-background/80 border border-border backdrop-blur-md shadow-lg
        ...
        <div className="flex justify-center mt-2">
          <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-semibold bg-emerald-500/10 border border-emerald-500/30 text-emerald-600 dark:text-emerald-400 backdrop-blur-md">
        ...
        {/* Description */}
        <p className="text-sm text-muted-foreground leading-relaxed text-center max-w-xs mx-auto">
        ...
        <div className="flex flex-wrap justify-center gap-2">
          {techStack.map((tech) => (
            <span
              key={tech}
              className="px-2.5 py-1 rounded-full text-[11px] font-medium border border-border bg-secondary/50 text-muted-foreground"
            >
              {tech}
            </span>
          ))}
      ...
      <div className="hidden md:grid max-w-6xl w-full mx-auto grid-cols-2 gap-16 items-center">
        ...
        <div className="flex flex-col gap-6 text-left">
          ...
          <div className="flex flex-wrap gap-2">
            {techStack.map((tech) => (
              <span
                key={tech}
                className="px-3 py-1 rounded-full text-xs font-medium border border-border bg-secondary/50 text-muted-foreground hover:bg-secondary hover:text-foreground transition-colors cursor-default"
              >
                {tech}
              </span>
            ))}
        ...
        <div className="flex justify-center">
          <div className="relative">
            <div className="absolute -inset-4 rounded-3xl bg-foreground/5 blur-2xl pointer-events-none" />
            <div className="relative w-96">
              <div className="relative w-full aspect-3/4 rounded-3xl overflow-hidden border border-border/50 shadow-2xl shadow-foreground/20">
                <Image
                  src="/images/my-pix.png"
                  alt="Profile picture"
                  fill
              ...
              {heroStats.map((stat, i) => (
                <div
                  key={stat.value}
                  className={`absolute flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-background/80 border border-border backdrop-blur-md shadow-xl
